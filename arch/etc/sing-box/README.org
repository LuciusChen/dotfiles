...
# 修复系统 DNS 环境后再更新
sudo ./sing-box-update.sh --repair-env "https://example.com/subscribe"
#+end_src
...
4. 可选修复 =systemd-resolved= / =NetworkManager= 的 DNS 环境
5. 验证配置并重启 sing-box
...
- =--repair-env= 修复 =systemd-resolved= / =NetworkManager= / =/etc/resolv.conf= 组合
- =-h, --help= 显示帮助

** repair-resolver-env.sh

一次性修复 Arch 上 sing-box TUN 依赖的 DNS 环境：

- 写入 =/etc/NetworkManager/conf.d/10-dns.conf=，启用 =dns=systemd-resolved=
- 启用并重启 =systemd-resolved=
- 将 =/etc/resolv.conf= 链接到 =/run/systemd/resolve/stub-resolv.conf=
- 输出修复后的 =resolvectl status=

#+begin_src bash
sudo ./repair-resolver-env.sh
#+end_src
...
- =ping 1.1.1.1= 正常，但域名解析失败
- =resolvectl status= 中出现 =resolv.conf mode: foreign=
- 启动 sing-box 后 =tun0= 成为默认 DNS，且解析异常

优先修复系统 DNS 环境，再重新生成配置：
